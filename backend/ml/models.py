"""
AeroTwin — Neural Network Models
Implements:
...
class LeakLocalizationNet(nn.Module):
    ...
    Classes: 0=No leak, 1=Zone 1, 2=Zone 2, 3=Zone 3, 4=Zone 4, 5=Zone 5, 6=Zone 6
    
    WARNING / ARCHITECTURAL LIMITATION (Single-Leak Assumption):
    This model maps inputs to a single output class via standard Softmax.
    As a result, it assumes that leak zones are mutually exclusive. It CANNOT
    represent or classify simultaneous multi-zone leaks. In scenarios where multiple
    leak zones fail at once, the network will isolate only the primary/most dominant
    zone distortion in the telemetry vector.
    """
