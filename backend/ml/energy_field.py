"""
AeroTwin — Energy Field Detector
...
# Default thermodynamic weighting matrix (fallback)
...
def compute_energy_field(sensor_window: np.ndarray, thermo_weights: np.ndarray = None) -> np.ndarray:
    """
    ...
    Args:
        ...
        thermo_weights: optional custom weighting matrix, shape (6, 6)

    Returns:
        energy_field of shape (6, 6) — normalized, weighted correlation matrix
    """
    # Step 1: Normalize only the flow-like channel (MAF at index 0) by RPM
    # (Pressures and temperatures are not speed-scaled physical flows)
    rpm = sensor_window[:, 6:7]
    rpm_safe = np.where(rpm < 100, 1800.0, rpm)
    
    normalized = sensor_window[:, :6].copy()
    normalized[:, 0] = normalized[:, 0] / (rpm_safe[:, 0] / 1800.0)
    ...
    weights = THERMO_WEIGHTS if thermo_weights is None else thermo_weights
    energy_field = corr_matrix[:6, :6] * weights
...
class EnergyFieldDetector:
    ...
    def __init__(self, engine_profile: str = "Cat C18"):
        ...
        self.thermo_weights = np.array(profile["thermo_weights"])
    ...
    def fit(self, healthy_windows: list):
        ...
        for w in healthy_windows:
            if w.shape[0] >= 5:  # Need at least 5 timesteps
                fields.append(compute_energy_field(w, self.thermo_weights))
    ...
    def compute_deviation(self, live_window: np.ndarray) -> dict:
        ...
        live_field = compute_energy_field(live_window, self.thermo_weights)
