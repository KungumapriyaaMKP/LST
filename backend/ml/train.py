...
def compute_features_fast(df: pd.DataFrame,
    ...
    """Compute 43-feature vector for the dataset (optimized)."""
    n = len(df)

    # Raw sensor features (19: 13 original + 6 extended)
    raw_cols = ['RPM', 'MAF', 'MAP_intake', 'MAP_boost', 'MAP_cac_in', 'MAP_cac_out',
                'T_intake', 'T_boost', 'T_cac_out', 'T_exh_manifold', 'T_dpf_in',
                'T_dpf_out', 'fuel_qty', 'turbo_speed', 'P_exh_manifold',
                'P_turbine_out', 'P_tailpipe', 'T_DOC', 'T_SCR']
    raw_features = df[raw_cols].values

    # Residual features (15: 9 original + 6 extended) — vectorized where possible
    ...
    for i in range(n):
        ...
        if 'turbo_speed_pred' in ca:
            residuals[i, 9] = row.get('turbo_speed', 0) - ca['turbo_speed_pred']
        ...
        if 'P_exh_manifold_pred' in ex:
            residuals[i, 10] = row.get('P_exh_manifold', 0) - ex['P_exh_manifold_pred']
        if 'P_turbine_out_pred' in ex:
            residuals[i, 11] = row.get('P_turbine_out', 0) - ex['P_turbine_out_pred']
        if 'P_tailpipe_pred' in ex:
            residuals[i, 12] = row.get('P_tailpipe', 0) - ex['P_tailpipe_pred']
        if 'T_DOC_pred' in ex:
            residuals[i, 13] = row.get('T_DOC', 0) - (ex['T_DOC_pred'] - 273.15)
        if 'T_SCR_pred' in ex:
            residuals[i, 14] = row.get('T_SCR', 0) - (ex['T_SCR_pred'] - 273.15)
    ...
    t_diff = np.abs(t_boost - t_intake) + 1e-8
    ratios[:, 1] = np.where(t_diff > 1.0, (t_boost - t_cac) / t_diff, 0.88)
...
def run_training():
    ...
    print("\n[3/7] Computing feature vectors (43 features: 19 raw + 15 residuals + 6 ratios + 3 stats)...")
    ...
    zone_names = ['Healthy', 'Zone 1', 'Zone 2', 'Zone 3', 'Zone 4', 'Zone 5', 'Zone 6']
